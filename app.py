...
def load_data():
    ...
    contractor_path = os.path.join(base_dir, "contractor_mapping_template.csv")
    if os.path.exists(contractor_path):
        contractor = pd.read_csv(contractor_path, dtype=str).fillna("")
        if "WorkOrderNum" in contractor.columns and "ContractorName" in contractor.columns:
            contractor["WorkOrderNum"] = contractor["WorkOrderNum"].astype(str).str.strip()
            contractor["ContractorName"] = contractor["ContractorName"].astype(str).str.strip()
            df = df.merge(contractor, on="WorkOrderNum", how="left")
            df["ContractorName"] = df["ContractorName"].replace("", np.nan)
    return df
...
st.markdown(f"""
    <style>
        ...
        .stMetric {{background-color: #F8F5FC; border: 1px solid #E9DDF8; border-radius: 14px; padding: 10px 14px;}}
        .title-box {{background: {PURPLE}; color: white; border-radius: 14px; padding: 16px 20px;}}
    </style>
""", unsafe_allow_html=True)

st.markdown('<div class="title-box"><h2 style="margin:0">STC Project Quality Executive Dashboard</h2><p style="margin:6px 0 0 0">Deviation hotspots • WO drilldown • district heatmap • KPI recovery lens</p></div>', unsafe_allow_html=True)

with st.sidebar:
    st.header("Filters")
    district_filter = st.multiselect("District", options=sorted(df["District"].dropna().unique()))
    deviation_filter = st.multiselect("Deviation", options=sorted(df["DeviationShort"].dropna().unique()))
    wo_filter = st.multiselect("Work Order", options=sorted(df["WorkOrderNum"].dropna().unique()))
    contractor_exists = "ContractorName" in df.columns and df["ContractorName"].notna().any()
    if contractor_exists:
        contractor_filter = st.multiselect("Contractor", options=sorted(df["ContractorName"].dropna().unique()))
    else:
        contractor_filter = []
        st.info("Contractor ranking is enabled once contractor names are filled in contractor_mapping_template.csv")
...
if contractor_filter and "ContractorName" in filtered.columns:
    filtered = filtered[filtered["ContractorName"].isin(contractor_filter)]
...
for i, row in kpi.iterrows():
    ...
    metric_cols[i].metric(row["Metric"], f'{row["Current"]:.2f}%', None if delta is None else f"{delta:.2f} pts vs target")
...
with colA:
    ...
    fig = px.bar(top_dev.sort_values("Count"), x="Count", y="Deviation", orientation="h", color_discrete_sequence=[PURPLE])
...
with colB:
    ...
    fig2 = go.Figure()
    fig2.add_trace(go.Scatter(x=monthly["Month"], y=monthly["Overall KPI %"], mode="lines+markers+text",
                              text=[f"{v:.2f}%" for v in monthly["Overall KPI %"]], textposition="top center",
                              line=dict(color="#7B61FF", width=3), name="Overall KPI"))
    fig2.add_trace(go.Scatter(x=monthly["Month"], y=monthly["Target %"], mode="lines",
                              line=dict(color="#9CA3AF", dash="dot"), name="Target"))
    fig2.update_layout(height=380, margin=dict(l=10, r=10, t=10, b=10), yaxis_title="KPI %")
...
with colC:
    ...
    if not heat.empty:
        fig3 = px.imshow(heat, text_auto=True, aspect="auto", color_continuous_scale=[[0, "#F3E8FF"], [1, PURPLE]])
        fig3.update_layout(height=430, margin=dict(l=10, r=10, t=10, b=10), coloraxis_showscale=False)
...
with colD:
    ...
    wo_rank = filtered.groupby(["WorkOrderNum", "District"]).size().reset_index(name="Deviation Count").sort_values("Deviation Count", ascending=False).head(15)
    fig4 = px.bar(wo_rank.sort_values("Deviation Count"), x="Deviation Count", y="WorkOrderNum", orientation="h",
                  color="District", color_discrete_sequence=[PURPLE, TEAL, ORANGE, PINK])
    fig4.update_layout(height=430, margin=dict(l=10, r=10, t=10, b=10))
    st.plotly_chart(fig4, use_container_width=True)

st.subheader("WO Drilldown")
wo_summary = (filtered.groupby(["WorkOrderNum", "District", "DeviationShort"])
                     .size()
...
for dev in cross.index:
    ...
    owner_rows.append({"Deviation": dev, "Highest District": district, "Count": count, "Share %": share})
st.dataframe(pd.DataFrame(owner_rows), use_container_width=True, hide_index=True)

st.subheader("Contractor Ranking")
if "ContractorName" in filtered.columns and filtered["ContractorName"].notna().any():
    contractor_rank = (filtered.groupby("ContractorName")
                             .size()
                             .reset_index(name="Deviation Count")
                             .sort_values("Deviation Count", ascending=False))
    fig5 = px.bar(contractor_rank.head(15).sort_values("Deviation Count"), x="Deviation Count", y="ContractorName",
                  orientation="h", color_discrete_sequence=[ORANGE])
    fig5.update_layout(height=420, margin=dict(l=10, r=10, t=10, b=10), showlegend=False)
    st.plotly_chart(fig5, use_container_width=True)
else:
    st.info("No contractor field exists in the current raw data. Fill contractor_mapping_template.csv to activate contractor ranking in this dashboard.")
